console.log("Content script load");

// Append Image for overlay
const imgElem = document.createElement("img");
imgElem.id = "overlay_img_icon";
imgElem.style = "width:50px; height:50px; position:fixed; display:none";
imgElem.src = "data:image/png;base64,";

document.body.appendChild(imgElem);

// event listener for the overlay img
function onClick(event){
	console.log(event.target.dataset.imgsrc);
	let createData = {
		// type: "detached_panel",
		type: "normal",
		// url: `https://www.google.com/search?tbm=isch&q=${event.target.dataset.imgsrc}`,
		url: `https://www.google.com/searchbyimage?image_url=${event.target.dataset.imgsrc}`,
		width: 250,
		height: 100
	};
	console.log(createData);
	browser.runtime.sendMessage({type:"search", createData:createData, rawUrl: event.target.dataset.imgsrc})
}
imgElem.addEventListener("click", onClick);

// event listener for all imgs
function onMouseOver(event){
	// console.log("mouse over");
	const boundingElem = event.target.getBoundingClientRect();
	const imgElem = document.getElementById("overlay_img_icon");
	imgElem.style.left = `${boundingElem.x}px`;
	imgElem.style.top = `${boundingElem.y}px`;
	imgElem.style.display = "block";
	imgElem.dataset.imgsrc = event.target.src;
}

function onMouseOut(event){
	// console.log("mouse out");
	imgElem.style.display = "none";
}

const allImgs = document.body.querySelectorAll("img");
allImgs.forEach(item=>{
	if (item != imgElem){
		console.log(item);
		item.addEventListener("mouseover", onMouseOver);
		item.addEventListener("mouseout", onMouseOut);
	}
});